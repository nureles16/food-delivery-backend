...
Модуль notifications отвечает за отправку уведомлений пользователям о статусе заказов, акциях и других событиях.
...
public class Notification {
    ...
    private UUID userId;           // получатель уведомления

    private String title;          // заголовок уведомления

    @Column(columnDefinition = "TEXT")
    private String message;        // содержание уведомления

    @Enumerated(EnumType.STRING)
    private NotificationType type; // тип уведомления: INFO, ALERT, PROMO
...
* userId связывает уведомление с пользователем
...
Репозиторий для CRUD операций уведомлений:
```
...
public class NotificationService {
    ...
    public Notification sendNotification(UUID userId, String title, String message, NotificationType type) {
...
```
Метод	URL	        Роль	Описание
POST	/notifications	send	ADMIN	Отправить уведомление пользователю
GET	/notifications/my	USER	Получить все непрочитанные уведомления
PATCH	/notifications/{id}/read	USER	Пометить уведомление как прочитанное
```
Пример запроса POST /notifications/send:
```
{
"userId": "uuid-пользователя",
"title": "Статус заказа",
"message": "Ваш заказ #123 подтверждён",
"type": "INFO"
}
```
Пример запроса PATCH /notifications/{id}/read:
```
{
"notificationId": "uuid-уведомления"
}
```
---
